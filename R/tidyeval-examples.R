...
summarise_var_new <-function(data, variable) {
	summarize(nlsy,
						median = median(income, na.rm = T),
						pctl_25 = quantile({{variable}}, p = 0.25, na.rm = TRUE),
						pctl_75 = quantile({{variable}}, p = 0.75, na.rn = TRUE))
}

summarise_var_new(nlsy, income)
summarise_var_new(nlsy, age_bir)
summarise_var_new(nlsy, nsibs)

# 2. Add a `group` argument using .by = {{ group }}, with a default so that
#    the function still works when you don't pass a group.

summarise_var_new <-function(data, variable, group = NULL) {
	summarize(nlsy,
						median = median(income, na.rm = T),
						pctl_25 = quantile({{variable}}, p = 0.25, na.rm = TRUE),
						pctl_75 = quantile({{variable}}, p = 0.75, na.rn = TRUE),
...
summarise_var_new(nlsy, income, region_cat)
summarise_var_new(nlsy, income)
...
summarize_two_var <- function(data, variable1, variable2) {
	summarize(nlsy,
						covariance = cov(income, age_bir, use = na/),
						correlation = cor(income, age_bir)

						cl = )
}

summarise_two_var(nlsy, income)
summarise_two_var(nlsy, age_bir)
summarise_two_var(nlsy, nsibs)
